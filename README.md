...
- **Standalone AVD Management & Background Creation**: Create brand new virtual devices (`[+ New AVD]`) directly from installed Android system images without opening Android Studio. Features non-blocking background creation with live animated placeholder cards in the sidebar and one-click quick boot (`[▶ Start Now]`). Launch emulators in detached high-speed mode, with Cold Boot or Wipe Data support.
